...
pub const Chunk = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, count: u8, capacity: u8) !Chunk {
        return .{ .allocator = allocator, .count = count, .capacity = capacity, .code = try allocator.alloc(u32, capacity), .constants = try value.ValueArray.init(allocator, count, capacity) };
    }
    ...
    pub fn writeChunk(self: *Chunk, byte: u32) !void {
